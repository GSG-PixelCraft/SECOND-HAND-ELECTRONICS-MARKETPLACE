...
import type { ListingReport } from "@/types/admin";

export interface ListingReportsTableProps {
...
export const ListingReportsTable = forwardRef<
  HTMLDivElement,
  ListingReportsTableProps
>(({ reports, className }, ref) => {
  const navigate = useNavigate();

  const handleRowClick = (report: ListingReport) => {
    navigate(getAdminReportDetailRoute("listing", report.id));
  };

  const handleViewClick = (e: MouseEvent, reportId: string) => {
    e.stopPropagation();
    navigate(getAdminReportDetailRoute("listing", reportId));
  };
  ...
  return (
    <div ref={ref} className={cn("overflow-hidden rounded-xl", className)}>
      <div className="flex h-12 items-center bg-primary/10">
        ...
        <div className="flex w-[280px] items-center gap-1 px-4">
      ...
      <div className="flex flex-col bg-white">
        {reports.map((report) => (
          ...
          >
            ...
            <div className="flex w-[280px] items-center gap-3 px-4">
              <Image
              ...
              />
              <Span className="text-neutral-70 max-w-[200px] truncate text-sm">
            ...
            <div className="flex w-[220px] items-center gap-2 px-4">
              <div className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-neutral-10">
                {report.listing.seller.avatar ? (
                  <Image
                    src={report.listing.seller.avatar}
                    alt={report.listing.seller.name}
                    className="h-full w-full object-cover"
                  />
                ) : (
                  <span className="text-xs font-semibold text-primary">
                    {report.listing.seller.name.charAt(0)}
                  </span>
                )}
              </div>
              <Span className="text-neutral-70 max-w-[120px] truncate text-sm">
                {report.listing.seller.name}
              </Span>
              {report.listing.seller.isVerified && (
                <BadgeCheck className="h-4 w-4 text-success" />
              )}
              {report.listing.seller.isFlagged && (
...
ListingReportsTable.displayName = "ListingReportsTable";
