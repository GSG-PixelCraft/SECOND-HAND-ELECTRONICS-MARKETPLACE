...
export const UserReportsTable = forwardRef<
  HTMLDivElement,
  UserReportsTableProps
>(({ reports, className }, ref) => {
  const navigate = useNavigate();

  const handleRowClick = (report: UserReport) => {
    navigate(getAdminReportDetailRoute("user", report.id));
  };

  const handleViewClick = (e: MouseEvent, reportId: string) => {
    e.stopPropagation();
    navigate(getAdminReportDetailRoute("user", reportId));
  };
...
UserReportsTable.displayName = "UserReportsTable";
